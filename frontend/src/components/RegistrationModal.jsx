...
const RegistrationModal = ({ moHopThoai, phongDangChon, dongHopThoai, xacNhanDangKy }) => {
    ...
    const handleTiepTuc = () => {
        if (buocHienTai === 0) {
            ...
            // Nếu có người đi cùng -> Chuyển sang form khách phụ 1
            ...
            // Nếu còn khách phụ chưa nhập -> Chuyển bước tiếp theo
            if (buocHienTai < parseInt(soNguoiChung)) {
                setBuocHienTai(prev => prev + 1);
            } else {
                // Đã là người cuối cùng -> Hoàn tất
                hoanTatDangKy();
            }
        }
    };
    ...
    const hoanTatDangKy = () => {
        const payload = {
            ...
            // --- LỖI TẠI TÔI, BÁC TRẢ LẠI 2 DÒNG NÀY LÀ XONG ---
            NhuCau: nhuCau,             
        ...
        // Gộp Khác vào Giới Tính cho danh sách khách phụ trước khi gửi
        const dsKhachPhuSuaDeGui = danhSachKhachPhu.map(k => ({
            ...k,
            GioiTinh: k.GioiTinh === 'Khác' ? k.GioiTinhKhac : k.GioiTinh
        }));

        // Trả về cả đại diện và tất cả khách phụ
        ...
        <div className="fixed inset-0 bg-[rgba(0,0,0,0.5)] flex justify-center items-center z-[100]">
            <div className="bg-white w-[600px] rounded shadow-2xl overflow-hidden flex flex-col">
                ...
                <div className="p-6 overflow-y-auto max-h-[80vh]">
                    ...
                    {/* BƯỚC 1 -> N: FORM NGƯỜI THUÊ CHUNG (Hiển thị tách biệt hệt như ảnh) */}
                    {buocHienTai > 0 && (
                        <div className="grid grid-cols-2 gap-x-6 gap-y-4">
                            <div>
                                <label className="block text-sm font-bold text-gray-800 mb-1">Họ và tên*</label>
                ...
                {/* --- FOOTER CHỨA NÚT BẤM THEO THIẾT KẾ --- */}
                <div className="px-6 py-4 flex justify-end gap-3 border-t border-gray-200 flex-shrink-0">
                    ...
                    {/* FOOTER KHÁCH PHỤ (Chưa phải người cuối) */}
                    {buocHienTai > 0 && buocHienTai < parseInt(soNguoiChung) && (
                        <>
                            {buocHienTai > 1 && (
                                <button onClick={handleQuayLai} className="bg-[#cbd5e1] text-black px-6 py-2 font-bold rounded hover:bg-gray-300 transition-colors">
                                    Quay lại
                                </button>
                            )}
                            <button onClick={handleTiepTuc} className="bg-[#cbd5e1] text-black px-6 py-2 font-bold rounded hover:bg-gray-300 transition-colors">
                                Tiếp
                            </button>
                        </>
                    )}

                    {/* FOOTER KHÁCH PHỤ CUỐI CÙNG */}
                    {buocHienTai > 0 && buocHienTai === parseInt(soNguoiChung) && (
                        <>
                            <button onClick={handleQuayLai} className="bg-[#cbd5e1] text-black px-6 py-2 font-bold rounded hover:bg-gray-300 transition-colors">
                                Quay lại
